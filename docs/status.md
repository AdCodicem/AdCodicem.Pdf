...
- **161 refused**: 68 for personal data — authors' e-mail addresses in 49 JHOVE files, named staff's
  ...
  AppleDouble fork that is not a PDF, and **all 89 of the iPRES 2017 hand-built set**: git stripped five carriage returns from the OPF
  copy of each of its 87 test files, as the authors' archive on RADAR (CC BY-SA 4.0) shows byte for byte.
  The originals are the best external test suite M2 could have, but RADAR serves them only as one tar; T26
  records what fetching them takes.
...
- **Afterwards**: the first two `Remote corpus` runs, recorded under *At a glance*. T25 stands as written,
  checked against `PdfFileReader.TryReadXRefChain`, which returns success when a later section fails as
  long as an earlier one was read. The RADAR lead became **T26**: the tar, downloaded again, hashed the
  same; it holds 88 test files — one more than the OPF's copy, `T04_019`, a trailer pointing at the wrong
  cross-reference offset — and neither reference file; RADAR's terms for data users ask nothing beyond the
  licence. M2 names the set as the input its structural profile is still missing.
...
| T26 | **The remote corpus cannot take a file out of an archive, so the one external test suite for M2's structural profile stays out of reach.** The iPRES 2017 hand-built set (Lindlar, Tunnat and Wilson, [doi:10.22000/53](https://doi.org/10.22000/53), CC BY-SA 4.0, so remote) is 88 one-page files, each breaking one ISO 32000-1 structural requirement, with a spreadsheet naming that requirement for each file and recording JHOVE 1.16.5's verdict and whether Acrobat XI opens it. The OPF's copies are unusable — git removed five bytes from each (`docs/corpus-sources.md`) — and RADAR serves the originals only inside one BagIt tar: `https://www.radar-service.eu/radar-backend/archives/JtlOdwQquZWDqQdq/versions/1/content`, 613,888 bytes, SHA-256 `33b2b4510f70aae3f4c606ca087292fc506759bb917e3b9bf9663d74ffca4f43`, byte-identical on two downloads nine hours apart on 2026-09-25, so it can be pinned. `fetch_remote.py` fetches one file per URL. RADAR's terms for data users (July 2019), which its web interface shows before a download, charge nothing and ask only that the dataset's licence be honoured; they say nothing of automated download, and the backend URL serves the tar without the prompt | Proposed: **before M2's slice 2**, so the file and cross-reference rules meet the set as they are written. (1) The manifest: `source.url` names the archive, `source.sha256` and `source.bytes` keep pinning the document itself — the hash `fetch_remote.py` checks on disk and `Remote_documents_are_pinned_and_kept_where_git_ignores_them` requires — and a new `source.archive` gives the archive's `sha256` and `bytes` and the `member` path. (2) `fetch_remote.py`: download each archive once per run and check its hash before opening it; stream out only the named member with `tarfile`, standard library only; refuse a member that is not a regular file or outgrows the pinned size; write it at the entry's `file` path, never at a path read from the archive, and never through `extractall`; then check the member's own hash, as today. `load_entries` and the C# test validate the new object. (3) The 88 files: a personal-data screen, then entries whose expectations come from qpdf (`build_corpus.py --remote`) — most recorded as unsupported until M2 — and the spreadsheet's requirement as the finding M2 should produce; JHOVE's verdict is a second opinion, not a referee. (4) `tests/corpus/README.md` and `docs/corpus.md` describe the field |
